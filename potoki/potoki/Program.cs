    ...
    class Program
    {
                ...
                if (Nums[i] == 0)
                {
                    for (int j = i + 1; j < Math.Sqrt(N); j++)
                        if (j % i == 0)//если j делится нацело на i, оно составное
                            Nums[j] = 1;
                    prchisla.Add(i);//добавляем простое число(делитель) в список
                }
            }
        }
        ...
        private static void ThrFuncT3(object obj)
        {
            ...
            for (int i = Sqrt; i < N + 1; i++)
                ...
                {
                    if (i % prchisla[j] == 0)//если i делится нацело на prchisla[j], оно составное
                        Nums[i] = 1;
                }
        }
        ...
        //– метод обработки всех чисел диапазона sqrt(n) …n на разложимость простому числу prchisla[i]
        ...
        private static void Func()
        {
            ...
            while (true)
            {
                ...
                lock ("Critical")
                {
                    current_prime = prchisla[current_index];//берем текущее простое число
                    current_index++;
                }
